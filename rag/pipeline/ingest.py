...
collectors/*.collect()가 만든 raw dict 1건을 ingest_document()에 넣으면:
    ...
    -> documents/chunks insert(원문 언어 그대로, 번역 없음) -> (EMBEDDING_PROVIDER 설정돼 있으면) chunk 임베딩 계산 후 저장
...
문서 1건당 관련성판정 LLM 호출은 정확히 1번(구성 안 돼 있으면 0번).

2026-09-17: "뉴스/판례/정책 등도 번역하지 말고 원문 그대로 저장·임베딩" 하기로 결정 — 예전엔
document_type != "paper"이고 language != "ko"인 문서(주로 NewsAPI발 해외 뉴스)만 LLM으로
한국어 번역했는데, 이제 paper와 동일하게 전부 원문 언어 그대로 저장함. 이유: EMBEDDING_MODEL이
애초에 다국어 검색을 위해 채택한 모델(text-embedding-3-large)이라 원문이어도 한국어 쿼리로
정상 검색되고(cross-lingual 임베딩), 번역 과정에서 생기는 미묘한 의미 왜곡·LLM 비용·품질
이슈(과거 backfill_retranslate.py가 다루던 "일부 문장이 번역 안 되고 원문 그대로 남는" 버그
등)를 아예 없앨 수 있음. rag/preprocessing/translate.py 자체는 삭제하지 않음 — 과거에 이미
번역돼 저장된 문서를 손보고 싶으면 rag/scripts/backfill_retranslate.py로 여전히 수동 실행 가능
(단, 이제는 새로 들어오는 문서엔 적용 안 됨). content_hash는 항상 저장되는 원문 그대로를
기준으로 계산됨.
...
def ingest_document(raw_doc: dict, sync_graph: bool = True) -> dict:
    ...
    with config.get_connection() as conn:
        ...
        # 2026-09-17: 번역 단계 제거 — 모든 document_type이 원문 언어 그대로 저장/임베딩됨
        # (paper와 동일한 정책으로 통일). 배경은 모듈 docstring 참고.
